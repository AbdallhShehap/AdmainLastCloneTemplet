...
export default function SecoundMarquee() {
  ...
  const handleSelection = (e) => {
    ...
    const selectedItem = marqueeData.find(item => item["id-secound-marquee"].toString() === selectedId);

    if (selectedItem) {
      setSelectedOption(selectedId);
      setText(selectedItem.titleSecoundMarquee);
      setSelectedImage(`http://localhost:1010/${selectedItem.imageSecoundMarqueePath}`);
    }
  };
  ...
  const DeletePost = async () => {
    if (selectedOption) {
      try {
        await axios.delete(`http://localhost:1010/secoundmarquee/secoundmarquee/${selectedOption}`);
        Toastify({
        ...
        setMarqueeData(marqueeData.filter(item => item["id-secound-marquee"].toString() !== selectedOption));
  ...
  const UpdatePost = async () => {
    ...
    formData.append('titleSecoundMarquee', text);
    if (selectedFile) {
      formData.append('imagesecoundmarquee', selectedFile);
    }

    try {
      await axios.put(`http://localhost:1010/Secoundmarquee/Secoundmarquee/${selectedOption}`, formData, {
        headers: {
  ...
  const AddPost = async () => {
    ...
    try {
      await axios.post('http://localhost:1010/secoundmarquee/add', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
          ...
          <FormGroup>
            ...
            >
              ...
              {marqueeData.map((item) => (
                <option key={item["id-secound-marquee"]} value={item["id-secound-marquee"]}>
