...
function YourComponent() {
  ...
  const manageSocialMedia = async (action) => {
    ...
    try {
      ...
      if (iconFile) formData.append('socialiconfooter', iconFile);


      switch (action) {
        case 'add':
          response = await axios.post('http://localhost:1010/footericons/addsocialiconfooter', formData);
          break;
        case 'update':
          response = await axios.put(`http://localhost:1010/footericons/socialiconfooter/${socialMedia.id}`, formData);
          break;
        case 'delete':
          response = await axios.delete(`http://localhost:1010/footericons/socialiconfooter/${socialMedia.id}`);
          break;
        default:
          return;
      }
